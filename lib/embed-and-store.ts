...
export function categoryToNamespace(category: string): string {
  ...
  if (/hostel|dormitor|accommodat|boarding|residence.?hall/i.test(cat)) return 'hostel'
  if (/exam|examination|result|grade|cgpa|gpa/i.test(cat)) return 'examination'
  if (/facility|facilities|transport|library|sports|cafeteria|lab/i.test(cat)) return 'facilities'
